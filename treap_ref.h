...
namespace treap {
    ...
    TreapVertex<DataType>* CycleShiftLeft(
        ...
        if (!shift || shift >= root->Size()) {
            return root;
        }
        auto [old_left, old_right] = SplitTreap(root, shift);
        root = MergeTreap(old_right, old_left);
        return root;
    }
    ...
    TreapVertex<DataType>* MoveToFirstPos(
        ...
        uint32_t v_pos = PosNumberInTreap(vertex);
        virtual_root = CycleShiftLeft(virtual_root, v_pos);
